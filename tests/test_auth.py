...
@pytest.mark.asyncio
async def test_callback_stores_session_and_redirects_to_frontend(client):
    ...
    assert response.status_code in (302, 307)
    # After login, redirects to FRONTEND_URL/dashboard
...
async def test_logout_clears_session(client):
    ...
    assert response.status_code == 200  # returns JSON
